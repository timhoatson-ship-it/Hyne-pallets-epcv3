...
# CORS: Allow origins from CORS_ORIGIN env var (comma-separated for multiple).
# Since the frontend is served from the same Flask app, CORS is only needed
# if you have a separate frontend deployment or use a custom domain.
_cors_origins = [o.strip() for o in os.environ.get("CORS_ORIGIN", "").split(",") if o.strip()]
if _cors_origins:
    CORS(app, origins=_cors_origins)
else:
    # No CORS_ORIGIN set — same-origin only (most secure default)
    CORS(app, origins=[])
...
def hash_pin(pin):
    """Hash a PIN using the same PBKDF2 method as passwords.
    PINs must never be stored in plaintext — a DB dump would expose them all.
    """
    if not pin:
        return None
    from werkzeug.security import generate_password_hash
    return generate_password_hash(str(pin), method='pbkdf2:sha256', salt_length=16)


def check_pin(stored_hash, pin):
    """Verify a PIN against its stored hash.
    Returns True if match, False otherwise. Handles None/empty gracefully.
    """
    if not stored_hash or not pin:
        return False
    from werkzeug.security import check_password_hash
    # All PIN hashes are PBKDF2 — no legacy fallback needed (PINs were plaintext before)
    if stored_hash.startswith("pbkdf2:"):
        return check_password_hash(stored_hash, str(pin))
    # Legacy: plaintext PIN still in DB (not yet migrated) — direct compare + flag for upgrade
    return stored_hash == str(pin)
...
def send_email_smtp(to_email, subject, body_text, body_html=None):
    ...
    try:
        ...
        logging.warning("SMTP send failed: %s", e)
        return (False, "Email delivery failed. Check SMTP configuration.")
...
""")
    ...
    if c.execute("SELECT COUNT(*) FROM users").fetchone()[0] == 0:
        ...
        users = [
            ...
            (None, None, hash_pin("123456"), "bob.floor1", "Bob Floor1", "floor_worker"),
            (None, None, hash_pin("234567"), "jim.floor2", "Jim Floor2", "floor_worker"),
            (None, None, hash_pin("345678"), "pete.floor3", "Pete Floor3", "floor_worker"),
...
def migrate_db():
    ...
    # Security: Hash any plaintext PINs still in the database.
    # After this migration, all PINs will be stored as PBKDF2 hashes.
    # Plaintext PINs are identified by NOT starting with 'pbkdf2:'.
    try:
        plaintext_pins = conn.execute(
            "SELECT id, pin FROM users WHERE pin IS NOT NULL AND pin != '' AND pin NOT LIKE 'pbkdf2:%'"
        ).fetchall()
        if plaintext_pins:
            for row in plaintext_pins:
                uid, raw_pin = row[0], row[1]
                conn.execute("UPDATE users SET pin=? WHERE id=?", [hash_pin(raw_pin), uid])
            conn.commit()
            logging.info("Migrated %d plaintext PINs to PBKDF2 hashes.", len(plaintext_pins))
    except Exception as e:
        logging.warning("PIN hash migration note: %s", e)
...
if not JWT_SECRET:
    ...
    sys.exit(1)
JWT_EXPIRY_SECONDS = 86400  # 24 hours (was 7 days — shortened for security)
...
def make_token(user_id, role):
    ...
    Returns a standard HS256-signed JWT string containing:
      ...
      - exp (expires): current time + JWT_EXPIRY_SECONDS (24 hours)
    """
...
# ---------------------------------------------------------------------------
# Database backup download (separate Flask route — returns file, not JSON)
# ---------------------------------------------------------------------------

@app.route("/api/admin/backup-download/<filename>")
def backup_download(filename):
    """Download a backup file. Executive auth required via query param."""
    ...
    try:
        ...
        if not user or row_to_dict(user)["role"] != "executive":
    ...
    import re as _re
    if not _re.match(r'^data_backup_\d{8}_\d{6}\.db$', filename):
    ...
    return send_from_directory(backup_dir, filename, as_attachment=True,
        download_name=filename, mimetype="application/x-sqlite3")
...
def dispatch(method, path, params, body, conn):
    ...
    if method == "POST" and path == "/auth/pin-login":
        ...
        allowed, remaining = check_rate_limit(conn, f"pin:{username}:{request.remote_addr}")
        if not allowed:
            return {"status": 429, "body": {"error": "Too many attempts. Try again later."}}
        # Fetch by username, then verify PIN hash (never query by plaintext PIN)
        row = conn.execute("SELECT * FROM users WHERE username=? AND is_active=1", [username]).fetchone()
        if not row:
            record_login_attempt(conn, f"pin:{username}:{request.remote_addr}", False)
            return {"status": 401, "body": {"error": "Invalid username or PIN"}}
        user = row_to_dict(row)
        stored_pin = user.get("pin") or ""
        if not check_pin(stored_pin, pin):
            record_login_attempt(conn, f"pin:{username}:{request.remote_addr}", False)
            return {"status": 401, "body": {"error": "Invalid username or PIN"}}
        # Auto-upgrade plaintext PIN to hash on successful login
        if stored_pin and not stored_pin.startswith("pbkdf2:"):
            conn.execute("UPDATE users SET pin=? WHERE id=?", [hash_pin(pin), user["id"]])
            conn.commit()
        token = make_token(user["id"], user["role"])
        record_login_attempt(conn, f"pin:{username}:{request.remote_addr}", True)
        user.pop("password_hash", None)
        user.pop("pin", None)
        return {"status": 200, "body": {"token": token, "user": user}}
    ...
    if method == "POST" and path == "/users":
        ...
        password = body.get("password")
        pin = hash_pin(body.get("pin")) if body.get("pin") else None
        ...
        try:
            ...
            logging.warning("Constraint violation: %s", e); return {"status": 409, "body": {"error": "A record with this data already exists or a constraint was violated."}}
    ...
    if m:
        ...
        if method == "PUT":
            ...
            if "pin" in body:
                fields.append("pin=?"); vals.append(hash_pin(body["pin"]) if body["pin"] else None)
            if not fields:
    ...
    if method == "GET" and path == "/orders":
        ...
        # Verification filters — allows frontend to show "needs attention" lists
        if params.get("is_verified") is not None:
            where.append("o.is_verified=?"); vals.append(int(params["is_verified"]))
        if params.get("needs_reverify") is not None:
            where.append("o.needs_reverify=?"); vals.append(int(params["needs_reverify"]))
    ...
    if m:
        ...
        if method == "POST":
            ...
            try:
                ...
                return {"status": 400, "body": {"error": "unit_price must be a valid number"}}
            if unit_price < 0:
                return {"status": 400, "body": {"error": "unit_price cannot be negative"}}
            if not sku_id and not sku_code and not product_name:
                return {"status": 400, "body": {"error": "At least one of sku_id, sku_code, or product_name is required"}}
            if sku_id:
    ...
    if m and method == "PUT":
        ...
        if "unit_price" in body:
            try:
                p = float(body["unit_price"]) if body["unit_price"] is not None else 0
                if p < 0:
                    return {"status": 400, "body": {"error": "unit_price cannot be negative"}}
            except (TypeError, ValueError):
                return {"status": 400, "body": {"error": "unit_price must be a valid number"}}
    ...
    if method == "POST" and path == "/production/qa-check":
        ...
        try:
            ...
            logging.error("Internal error: %s", e); return {"status": 500, "body": {"error": "An internal error occurred. Please try again."}}
    ...
    # PINs are now hashed (PBKDF2). Since this is PIN-only login (no username),
    # we fetch all active users with a PIN set and verify each hash. With a small
    # user base (<50 drivers), this is efficient. PIN uniqueness is still enforced
    # at the application level in user management.
    if method == "POST" and path == "/driver/pin-login":
        pin = body.get("pin", "").strip()
        if not pin or len(pin) != 6:
            return {"status": 400, "body": {"error": "6-digit PIN required"}}
        allowed, remaining = check_rate_limit(conn, f"driverpin:{request.remote_addr}")
        if not allowed:
            return {"status": 429, "body": {"error": "Too many attempts. Try again later."}}
        # Fetch all active users that have a PIN set, then check each hash
        rows = conn.execute("SELECT * FROM users WHERE pin IS NOT NULL AND pin != '' AND is_active=1").fetchall()
        matched_user = None
        for r in rows:
            u = row_to_dict(r)
            if check_pin(u.get("pin", ""), pin):
                matched_user = u
                break
        if not matched_user:
            record_login_attempt(conn, f"driverpin:{request.remote_addr}", False)
            return {"status": 401, "body": {"error": "Invalid PIN"}}
        # Auto-upgrade plaintext PIN to hash on successful login
        stored = matched_user.get("pin", "")
        if stored and not stored.startswith("pbkdf2:"):
            conn.execute("UPDATE users SET pin=? WHERE id=?", [hash_pin(pin), matched_user["id"]])
            conn.commit()
        token = make_token(matched_user["id"], matched_user["role"])
        record_login_attempt(conn, f"driverpin:{request.remote_addr}", True)
        # Get default truck for this driver
        truck = conn.execute("SELECT * FROM trucks WHERE driver_name=? AND is_active=1",
                             [matched_user["full_name"]]).fetchone()
        matched_user.pop("password_hash", None)
        matched_user.pop("pin", None)
        result = {"token": token, "user": matched_user}
        if truck:
    ...
    if method == "GET" and path == "/trackmyride/position":
        ...
        try:
            ...
            return {"status": 200, "body": {"source": "manual", "message": "TrackMyRide API unavailable. Using manual logbook.", "error": "API connection failed"}}
        ...
        for action in actions:
            ...
            try:
                if action_type == "stage_start":
                    ...
                    # Convenience alias — refuel is a logbook entry with event_type=refuel
                    cur = conn.execute("""INSERT INTO driver_logbook
                        (driver_shift_id, delivery_log_id, event_type, odometer_reading, manual_km, location_lat, location_lng, location_description, notes, recorded_at)
                        VALUES (?,?,?,?,?,?,?,?,?,?)""",
                        [action_data.get("shift_id"), action_data.get("delivery_log_id"), "refuel",
                         action_data.get("odometer_reading"), action_data.get("manual_km"),
                         action_data.get("lat"), action_data.get("lng"), action_data.get("location_description", ""),
                         action_data.get("notes", ""), action_data.get("recorded_at", datetime.now(timezone.utc).isoformat())])
                    conn.commit()
                    results.append({"action_id": action.get("id"), "status": "ok", "server_id": cur.lastrowid})
                elif action_type == "delivery_status":
                    # Update delivery log status (e.g. in_transit, at_customer)
                    dl_id = action_data.get("delivery_log_id")
                    new_status = action_data.get("status")
                    if dl_id and new_status:
                        conn.execute("UPDATE delivery_log SET status=?, updated_at=? WHERE id=?",
                            [new_status, action_data.get("updated_at", datetime.now(timezone.utc).isoformat()), dl_id])
                        conn.commit()
                        results.append({"action_id": action.get("id"), "status": "ok"})
                    else:
                        results.append({"action_id": action.get("id"), "status": "error", "error": "delivery_log_id and status required"})
                elif action_type == "delivery_complete":
                    # Offline delivery completion — saves basic data. Cost calculation
                    # runs when driver app reconnects and calls the full endpoint.
                    dl_id = action_data.get("delivery_log_id")
                    if not dl_id:
                        results.append({"action_id": action.get("id"), "status": "error", "error": "delivery_log_id required"})
                    else:
                        now = action_data.get("completed_at", datetime.now(timezone.utc).isoformat())
                        conn.execute("UPDATE delivery_log SET status='delivered', actual_delivery_at=?, signed_by=?, notes=COALESCE(notes,'') || ?, updated_at=? WHERE id=?",
                            [now, action_data.get("signed_by", ""), "\n[Completed offline]", now, dl_id])
                        # Save proof-of-delivery photo if included
                        if action_data.get("photo_data"):
                            conn.execute("""INSERT INTO delivery_photos
                                (delivery_log_id, driver_shift_id, photo_type, photo_data, caption, location_lat, location_lng, taken_at)
                                VALUES (?,?,?,?,?,?,?,?)""",
                                [dl_id, action_data.get("shift_id"), "pod", action_data.get("photo_data"),
                                 action_data.get("caption", "Proof of delivery (offline)"),
                                 action_data.get("lat"), action_data.get("lng"), now])
                        conn.commit()
                        sse_notify("dispatch")
                        results.append({"action_id": action.get("id"), "status": "ok"})
                else:
                ...
                conn.rollback()
                logging.warning("Batch sync action failed: %s", e)
                results.append({"action_id": action.get("id"), "status": "error", "error": "Operation failed. Will retry on next sync."})
    ...
    # ----- DATABASE BACKUP -----
    # Creates a safe backup copy of the SQLite database using the backup API
    # (ensures a consistent snapshot even if writes are happening).
    # Executive-only. The backup file is saved to disk and can be downloaded
    # via the /admin/backup-download route (handled separately in Flask).
    if method == "POST" and path == "/admin/backup":
        if not current_user or current_user["role"] != "executive":
            return {"status": 403, "body": {"error": "Executive access required"}}
        import shutil
        backup_dir = os.path.join(os.path.dirname(DB_PATH), "backups")
        ...
        try:
            ...
            # Keep only the last 7 backups to avoid filling disk
            existing = sorted([f for f in os.listdir(backup_dir) if f.startswith("data_backup_")])
            while len(existing) > 7:
                os.remove(os.path.join(backup_dir, existing.pop(0)))
            log_audit(conn, current_user["id"], "database_backup", None, None, None, {"file": backup_path, "size_mb": size_mb})
            return {"status": 200, "body": {"message": "Backup created", "file": f"data_backup_{timestamp}.db", "size_mb": size_mb, "timestamp": timestamp}}
        except Exception as e:
            logging.error("Backup failed: %s", e)
            return {"status": 500, "body": {"error": "Backup failed. Check server logs."}}
    ...
    if method == "GET" and path == "/admin/backups":
        ...
        for f in sorted(os.listdir(backup_dir), reverse=True):
            if f.startswith("data_backup_") and f.endswith(".db"):
                ...
                files.append({"file": f, "size_mb": round(os.path.getsize(fpath) / (1024 * 1024), 2),
                    "created": datetime.fromtimestamp(os.path.getctime(fpath), tz=timezone.utc).isoformat()})
    ...
    if method == "GET" and path == "/ops/dashboard":
        ...
        # Orders needing verification or re-verification
        unverified_count = conn.execute(
            "SELECT COUNT(*) FROM orders WHERE is_verified=0 AND status NOT IN ('cancelled','archived')"
        ).fetchone()[0]
        reverify_count = conn.execute(
            "SELECT COUNT(*) FROM orders WHERE needs_reverify=1 AND status NOT IN ('cancelled','archived')"
        ).fetchone()[0]
        if unverified_count > 0:
            alerts.append({"type": "warning", "message": f"{unverified_count} order{'s' if unverified_count != 1 else ''} awaiting verification"})
        if reverify_count > 0:
            alerts.append({"type": "warning", "message": f"{reverify_count} order{'s' if reverify_count != 1 else ''} need re-verification (details changed after verification)"})
        ...
        return {"status": 200, "body": {
            ...
            "verification": {
                ...
                "needs_reverification": reverify_count
            },
